# AutoSfera AI — помощник для автосалона

AutoSfera AI принимает вопросы клиентов и сотрудников и направляет их в нужный отдел: продажи, поддержку, сервис или внутреннему помощнику. Ответы формируются по базе знаний, а важные действия — например, запись на тест-драйв или в сервис — выполняются только после подтверждения сотрудником.

Это **учебная контролируемая бета 3.1.0-beta.1** для одного вымышленного автосалона. Её можно запустить локально без API-ключа и показать на защите. Реальные CRM/DMS, каналы связи и production-обработка персональных данных пока не подключены.
...
- заявки, роли сотрудников, аналитика и оценка полезности ответа;
- безопасная очередь внешних действий: предложение → подтверждение человеком → выполнение → аудит;
- PostgreSQL для пилота и SQLite для локальной разработки;
- полностью автономный mock-режим для демонстрации без внешней LLM.

## Как система отвечает

1. Пользователь пишет сообщение.
2. Оркестратор определяет тему, сложность и риск.
3. Запрос получает Sales, Support, Service или Employee Agent.
4. Агент использует только разрешённые материалы базы знаний.
5. Если данных недостаточно или решение ответственное, система задаёт уточнение либо подключает сотрудника.
6. Любое внешнее действие требует подтверждения уполномоченного человека.
...
Откройте **http://127.0.0.1:8000**. Состояние приложения можно проверить по адресам **/health** и **/ready**.

### С PostgreSQL в Docker

```bash
cp .env.example .env
# Замените демонстрационные пароли и секреты.
...
Используйте 5 профильных вопросов и 2 вопроса вне темы. В ответе API проверяйте `agent`, `skill`, `model_route`, `rag_ids`, `escalated` и `routing_reason`. Не используйте реальные персональные данные и не показывайте файл `.env`.

- [Окончательное техническое задание](docs/TECHNICAL_SPECIFICATION_FINAL.md)
- [Пояснительная записка](docs/EXPLANATORY_NOTE_FINAL.md)
- [Промпт для подготовки демонстрационного видео в Cursor](docs/CURSOR_DEMO_VIDEO_PROMPT.md)
- [Матрица готовности беты](docs/BETA_READINESS.md)
- [Спецификация пилота](docs/COMMERCIAL_PILOT.md)

## Честные ограничения

Проект готов для учебной демонстрации и контролируемой беты, но не для обработки реальных клиентов в production. До промышленного запуска нужно подключить и проверить CRM/DMS, откалибровать pgvector на пилотных данных, ротировать секреты, настроить мониторинг и резервное восстановление, а также провести проверки безопасности, персональных данных и доступности интерфейса.
...
Фиксированный набор из 28 типичных, сложных, внешних и провокационных запросов запускается командой `python scripts/evaluate_rag.py`. Методика и пороги: [`docs/RAG_EVALUATION.md`](docs/RAG_EVALUATION.md).
