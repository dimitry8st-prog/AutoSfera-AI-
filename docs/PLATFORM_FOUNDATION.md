...
Единый Action Gateway реализован как защищённый следующий слой. Он использует
durable outbox, явное подтверждение сотрудником, HMAC, идемпотентность и аудит.
n8n по-прежнему не получает прямой доступ к таблицам приложения. Подробности:
