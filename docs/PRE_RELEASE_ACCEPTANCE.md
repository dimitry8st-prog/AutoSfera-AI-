...
| Технический пакет | `autonova` (публичное имя AutoSfera AI) |
...
**READY** для контролируемой сдачи MVP на текущей ветке `codex/pre-release-acceptance` при запуске через Docker Compose + PostgreSQL + OpenAI `gpt-4o-mini`.
