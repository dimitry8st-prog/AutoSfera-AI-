# Этап 3 Финальная приёмка бета-версии AutoSfera AI

Результат: 13/13 сценариев соответствуют целевому поведению.

- Версия: `3.1.0-beta.1`
- Commit: `aab2627`
- Промпты: `2026-09-13-beta.1` / `90cbbf22ad63`
- База знаний: `2026-09-13-beta.1` / `5827e9bd1fe9`
- LLM: `mock`
- RAG: `tfidf`

| Сценарий | Статус | Диагностика |
|---|---|---|
| catalog | PASS | Ожидаемый результат получен |
| order_status | PASS | Ожидаемый результат получен |
| warranty | PASS | Ожидаемый результат получен |
| service_booking | PASS | Ожидаемый результат получен |
| employee_process | PASS | Ожидаемый результат получен |
| switch_service_to_purchase | PASS | Ожидаемый результат получен |
| switch_sales_to_service | PASS | Ожидаемый результат получен |
| ambiguous_conditions | PASS | Ожидаемый результат получен |
| ambiguous_paperwork | PASS | Ожидаемый результат получен |
| off_scope_weather | PASS | Ожидаемый результат получен |
| illegal_request | PASS | Ожидаемый результат получен |
| graph_failure | PASS | Ожидаемый результат получен |
| restart_session | PASS | Ожидаемый результат получен |
